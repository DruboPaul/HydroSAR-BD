...
function runMonthly() {
  ...
  try {
    ...
    if (selMonth === 'Total Occurrence') {
      ...
      resultBox.setValue('Total occurrence: ' + selYear + ' | Export Ready');
    } else {
      ...
      drawRegionBoundaryOnly(regionInfo.feature, regionInfo.name + ' Boundary');
      resultBox.setValue('Water: ' + selMonth + ' ' + selYear + ' (S1: ' + count + ') | Export Ready');
    }
  } catch (err) {
...
function runChange() {
  ...
  try {
    ...
    if (c1 === 0 && c2 === 0) {
      resultBox.setValue(' No Sentinel-1 images for selected month in both years.');
      return;
    }
