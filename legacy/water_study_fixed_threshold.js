/* ==================================================================================
   SAR-based Surface Water Explorer for Bangladesh (Enhanced)
   
   Features:
   - Division / District selection (Bangladesh)
   - Monthly Surface Water detection (Sentinel-1 VV)
   - Total Occurrence with color gradient
   - Change detection between years
   - Seasonal water analysis
   - GeoTIFF Export (3 formats: Multi-band, Split, ColorMap)
   - Charts (Monthly, Change, Seasonal)
================================================================================== */
...
var occurrencePalette = [
  'FFFFFF', 'c6ecc6', 'a8e6a0', '7dd87d', '52c569',
  '3dbfb8', '40b0c0', '2f9ac4', '2171b5', '1565c0',
  '0d47a1', '0a3d91', '0000FF'
];
...
/* Build 12-band monthly water image for GeoTIFF export */
...
function runMonthly() {
  ...
  try {
    // Build 12-band monthly image for GeoTIFF export
...
function runChange() {
  ...
  try {
    ...
    if (c1 === 0 && c2 === 0) {
      resultBox.setValue('⚠️ No Sentinel-1 images for selected month in both years.');
      return;
    }
    if (c1 === 0) { resultBox.setValue('⚠️ No images for Year1 (' + y1 + ') — result may be partial.'); }
    if (c2 === 0) { resultBox.setValue('⚠️ No images for Year2 (' + y2 + ') — result may be partial.'); }
    ...
    drawRegionBoundaryOnly(regionInfo.feature, regionInfo.name + ' Boundary');
    resultBox.setValue('Change: ' + y1 + ' → ' + y2 + ' (' + m + ') — images (y1:' + c1 + ', y2:' + c2 + ')');
  } catch (err) {
...
function runSeasonal() {
  ...
  try {
    ...
    drawRegionBoundaryOnly(regionInfo.feature, regionInfo.name + ' Boundary');
    resultBox.setValue('Seasonal union: ' + s + ' ' + y + ' (images: ' + totalImages + ')');
  } catch (err) {
    ...
    if (y1 && y2 && changeMonth) {
      ...
      var chart = ui.Chart.array.values(arr, 0, labels)
        ...
        .setOptions({
          ...
          vAxis: { title: 'Area (km²)' }, hAxis: { title: 'Change class' },
          legend: { position: 'none' }, fontSize: 12
        });
    ...
    if (seasonalY && seasonalS) {
      ...
      var chart = ui.Chart.array.values(seasonArr, 0, ee.List(seasonNames))
        ...
        .setOptions({
          ...
          vAxis: { title: 'Area (km²)' }, hAxis: { title: 'Season' },
          legend: { position: 'none' }, fontSize: 12
        });
      chartPanel.clear(); chartPanel.add(chart);
    ...
    if (monthlyY) {
      ...
      var chart = ui.Chart.array.values(monthArr, 0, monthLabels)
        ...
        .setOptions({
          ...
          vAxis: { title: 'Area (km²)' }, hAxis: { title: 'Month' },
          legend: { position: 'none' }, fontSize: 11, bar: { groupWidth: '60%' }
        });
      chartPanel.clear(); chartPanel.add(chart);
...
/* -------------------- GeoTIFF EXPORT (3 Format Options) -------------------- */
function runGeoTIFFExport() {
  ...
  if (format === 'Multi-band (Single File)') {
    ...
    chartPanel.add(ui.Label('👉 Open Tasks tab → Click RUN', { color: 'blue', fontWeight: 'bold' }));
    resultBox.setValue('✅ Multi-band export @ ' + resolutionStr + ' | Check Tasks tab');
  }
  ...
  // ============ FORMAT 2: Single-band Split (13 Files) ============
    ...
    months.forEach(function (month, i) {
      ...
      var fName = 'Water_' + regionInfo.name + '_' + year + '_' + monthShort[i] + '_' + resolutionStr;
    ...
    chartPanel.add(ui.Label(''));
    chartPanel.add(ui.Label('👉 Open Tasks tab → Click RUN ALL', { color: 'blue', fontWeight: 'bold' }));
    resultBox.setValue('✅ 13 single-band files @ ' + resolutionStr + ' | Check Tasks tab');
  }
  ...
  // ============ FORMAT 3: Single-band with ColorMap (ArcMap-ready) ============
    ...
    months.forEach(function (month, i) {
      ...
      var visualized = monthlyBand.visualize({
        min: 0, max: 1,
        palette: ['FFFFFF', '0066FF']
      });
      var fName = 'WaterRGB_' + regionInfo.name + '_' + year + '_' + monthShort[i] + '_' + resolutionStr;
    ...
    chartPanel.add(ui.Label('Occurrence: gradient color ramp'));
...
function aboutAction() {
  ...
  ap.add(ui.Label('SAR-based Surface Water Explorer for Bangladesh', { fontWeight: 'bold', fontSize: '16px' }));
  ap.add(ui.Label(''));
  ap.add(ui.Label('Features:', { fontWeight: 'bold' }));
  ap.add(ui.Label('• Division / District selection'));
  ap.add(ui.Label('• Monthly water detection (Sentinel-1 VV)'));
  ap.add(ui.Label('• Total Occurrence with color gradient'));
  ap.add(ui.Label('• Change detection between years'));
  ap.add(ui.Label('• Seasonal water analysis'));
  ap.add(ui.Label('• GeoTIFF Export (3 formats)'));
  ap.add(ui.Label('• Charts (Monthly, Change, Seasonal)'));
  ap.add(ui.Label(''));
  ap.add(ui.Label('How to Export GeoTIFF:', { fontWeight: 'bold' }));
  ap.add(ui.Label('1. Select Division/District (or leave blank for Bangladesh)'));
  ap.add(ui.Label('2. Select Year + Month → Click "Monthly Surface Water"'));
  ap.add(ui.Label('3. Choose Export Format & Resolution'));
  ap.add(ui.Label('4. Click "GeoTIFF Download"'));
  ap.add(ui.Label('5. Open Tasks tab (top-right) → Click RUN'));
  ap.add(ui.Label('6. File exports to Google Drive'));
  ap.add(ui.Label(''));
