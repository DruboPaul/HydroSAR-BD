<<<<<<< HEAD:legacy/generate_charts.py
import pandas as pd
...
DATA_PATH = os.path.join(BASE_DIR, "data", "GEE_data", "computed_results", "national_monthly_water_area_2015_2025.csv")
df_master = pd.read_csv(DATA_PATH)
# Normalize columns for the script
nat_df = df_master.rename(columns={'month': 'Month', 'water_area_calibrated_km2': 'Area_km2'})
...
def fig4_monthly_timeseries():
    ...
    ax.set_xticklabels(months)
    ax.set_title('Monthly Surface Water Area in Bangladesh (2015-2025)')
    ax.set_ylim(8000, 52000)
    ax.legend(loc='upper left', framealpha=0.9)
    ax.grid(True, alpha=0.2, linestyle='--')

    # Add annotation for peak
    peak_row = nat_df.loc[nat_df['Area_km2'].idxmax()]
    ax.annotate("Peak: {:,} km2\n({:.1f}% of land)".format(int(peak_row['Area_km2']), peak_row['Area_km2']/147570*100),
                xy=(peak_row['Month']-1, peak_row['Area_km2']), xytext=(4, 49000),
                fontsize=9, ha='center',
...
def fig7_seasonal_bar():
    ...
    # Main bars
    colors = ['#4393c3', '#92c5de', '#d6604d', '#fdae61']
    bars = ax.bar(x, mean_area, width, color=colors, edgecolor='#333333',
                  linewidth=0.8, zorder=3)
    ...
    yerr_upper = [ma - m for m, ma in zip(mean_area, max_area)]
    ax.errorbar(x, mean_area, yerr=[yerr_lower, yerr_upper],
                fmt='none', ecolor='#333333', capsize=5, capthick=1.5, linewidth=1.5, zorder=4)

    # Value labels
    for bar, val in zip(bars, mean_area):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 800,
                "{:,} km2".format(int(val)), ha='center', va='bottom', fontsize=10, fontweight='bold')
    ...
    for bar, val in zip(bars, mean_area):
        pct = val / total_area * 100
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() / 2,
                "{:.1f}%".format(pct), ha='center', va='center', fontsize=10,
                color='white', fontweight='bold')

    ax.set_xticks(x)
    ax.set_xticklabels(seasons_labels)
    ax.set_ylabel('Mean Water Area (km2)')
    ax.set_title('Seasonal Surface Water Extent in Bangladesh (2015-2025 Average)')
    ax.set_ylim(0, 50000)
    ax.grid(axis='y', alpha=0.2, linestyle='--', zorder=0)
...
def fig9_decadal_trend():
    ...
    # Trend line
    ax.plot(years, trend_line, '--', color='#d62728', linewidth=2.5,
            label='Linear trend: {:.1f} km2/yr (R2 = {:.3f})'.format(slope, r_squared))

    # Mean line
    mean_val = np.mean(july_area)
    ax.axhline(y=mean_val, color='#666666', linestyle=':', alpha=0.5, linewidth=1)
    ax.text(2025.3, mean_val, 'Mean: {:,.0f} km2'.format(mean_val), fontsize=9, color='#666666', va='center')
...
def fig10_accuracy():
    ...
    for idx, (name, tp, fp, fn, tn) in enumerate(scenarios):
        ...
        ax.set_title('{}\nOA = {:.1f}%'.format(name, oa), fontsize=10, fontweight='bold')
        ax.set_xlabel('Predicted (SAR)', fontsize=9)
        ax.set_ylabel('Reference (JRC)', fontsize=9)
...
if __name__ == '__main__':
    ...
    print('=' * 60)
>>>>>>> 2f4a07f (Finalize methodologies, figures, datasets, and scripts for manuscript submission):scripts/generate_charts.py
