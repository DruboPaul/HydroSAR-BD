<<<<<<< HEAD:legacy/plot_vv_ndwi_intersection.py
import numpy as np
...
# Plot the optimal threshold line
plt.axvline(optimal_threshold, color='red', linestyle='-', linewidth=2.5, 
            label='Optimal Threshold (Intersection) $\\approx$ {0:.1f} dB'.format(optimal_threshold))
plt.plot(optimal_threshold, float(intersect_y), 'ro', markersize=8)
...
import os
save_dir = os.path.join(os.path.dirname(__file__), '..', '7_Manuscript', 'figures')
os.makedirs(save_dir, exist_ok=True)
art_path = os.path.join(save_dir, 'fig6_validation_plot.png')
plt.savefig(art_path, dpi=300)
print("Saved figure to: " + art_path)
>>>>>>> 2f4a07f (Finalize methodologies, figures, datasets, and scripts for manuscript submission):scripts/plot_vv_ndwi_intersection.py
